		...
		<div class="card card-custom gutter-b col-md-12">
			<div class="card-header flex-wrap border-0 pt-6 pb-0">
				<div class="card-title">
					<h3 class="card-label">{{ isset($data) ? "Update" : "Create New" }} Brand</h3>
				</div>
			</div>
			<form method="POST" enctype="multipart/form-data" action="{{ isset($data) ? route('brands.update', $data['id']) : route('brands.store') }}">
				@csrf
				@if(isset($data))
				@method('PUT')
				@endif
				<div class="card-body">
					<div class="form-group">
						<label>Name *</label>
						<input type="text" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="Brand Name" value="{{ old('name') ?? (isset($data) ? $data['name'] : '') }}" autocomplete="off" />
						@error('name')
